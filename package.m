...
TestManipulate[] :=
    Column[{Manipulate[
        If[Or[x == "KMeans", x == "KMedoids", x == "Spectral"](*if per il metodo selezionato*),
        ...
        Row[{Control[{{x, "KMeans", "Method"}, {"MeanShift" -> "Mean Shift", "GaussianMixture" -> "Gaussian Mixture", "NeighborhoodContraction" -> "Neighborhood Contraction"}}](*selezione metodo clustering senza numero di cluster predefinito*)}],
    ...
    Manipulate[
        If[edge == True,
            imgDrop = ImageTake[ImageAdd[imgA // Colorize, EdgeDetect[imgA // Colorize]], {top, bottom}, {left, right}](*con edge detection selezionata, viene applicata image take che va a partizionare l'immagine attraverso i valori definiti dalla manipulate, verr\[AGrave] poi applicata edge detection su immagine partizionata*)
            ,
            imgDrop = ImageTake[imgA // Colorize, {top, bottom}, {left, right}](*casistica in cui edge detection non \[EGrave] selezionato*)
